...
export default function BoardLayout({
    ...
    return (
        <div className="min-h-screen flex flex-col transition-colors duration-300">
            <header className="h-20 glass-panel z-50 sticky top-0 px-4 md:px-8 flex items-center justify-between border-b border-border shadow-md">
                {/* Left Side: Team & Projects */}
                <div className="flex items-center gap-3">
                    <div className="relative">
                        <button
                            onClick={() => setIsTeamMenuOpen(!isTeamMenuOpen)}
                            className="flex items-center gap-2 md:gap-3 bg-secondary/30 hover:bg-secondary/50 p-1.5 md:px-4 md:py-2.5 rounded-[20px] md:rounded-2xl border border-border shadow-sm transition-all active:scale-95 group"
                        >
                            <div className="h-9 w-9 md:h-10 md:w-10 bg-primary/10 border border-primary/20 rounded-xl flex items-center justify-center text-primary font-black shadow-sm group-hover:scale-105 transition-transform">
                                {team.name.charAt(0)}
                            </div>
                            <div className="hidden sm:block text-right">
                                <div className="flex items-center gap-2">
                                    <h1 className="font-extrabold text-foreground leading-tight text-sm truncate max-w-[120px]">{team.name}</h1>
